...
describe('studio blocker handoff source shell', () => {
  it('matches dashboard terminal CLI args for analyze and pipeline', () => {
    expect(CARD_SOURCE_SHELL.analyze).toBe(
      `npx workspai ${DASHBOARD_COMMAND_CONTRACTS.workspaceAnalyze.cliArgs?.join(' ')}`
    );
    expect(CARD_SOURCE_SHELL.pipeline).toBe(
      `npx workspai ${DASHBOARD_COMMAND_CONTRACTS.workspacePipeline.cliArgs?.join(' ')}`
    );
  });

  it('pins doctor, verify, impact, and explain shells for Studio handoff', () => {
    expect(CARD_SOURCE_SHELL.doctor).toBe('npx workspai doctor workspace --json');
    ...
    expect(CARD_SOURCE_SHELL.contract).toBe(
      `npx workspai ${DASHBOARD_COMMAND_CONTRACTS.workspaceContractVerify.cliArgs?.join(' ')}`
    );
