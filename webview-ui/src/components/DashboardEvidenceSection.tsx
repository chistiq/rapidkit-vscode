...
export function DashboardEvidenceSection({
    ...
    const hasEvidenceContent =
        cards.length > 0 || activity.length > 0 || hasWorkspace;
    ...
    if (!hasEvidenceContent) {
        return (
            <WorkspaiEmptyState
                ...
                description={
                    <>
                        Select a workspace to populate the ops evidence loop. Artifacts from doctor,
                        analyze, readiness, and release commands appear here.
                    </>
                }
                actions={
                    <>
                        ...
                        <button
                            type="button"
                            className="ws-btn"
                            onClick={() => onNavigateSection('operate')}
                        >
                            Open Operate
                        </button>
                    </>
    ...
    if (sparseWorkspaceEvidence) {
        return (
            <WorkspaiEmptyState
                ...
                description={
                    <>
                        This workspace has no doctor, analyze, or readiness artifacts yet. Run the
                        bootstrap → doctor → analyze chain from Operate to populate evidence.
                    </>
                }
                actions={
                    <>
