...
const cardCommandMap = EVIDENCE_CARD_COMMANDS;
    ...
    return (
        <section className="command-activity-panel" aria-label="Command activity and evidence">
            <div className="command-activity-panel__head">
                ...
                <span className="command-activity-panel__meta">
                    Command → artifact → outcome → next step
                </span>
                {activity.length > 0 && onClearActivity ? (
                    <button
                        type="button"
                        className="command-activity-panel__clear"
            ...
            {cards.length > 0 ? (
                <div className="command-activity-panel__evidence">
                    {cards.map((card) => {
                        ...
                        const runCommand = cardCommandMap[card.id];
                        return (
                            <button
                                key={card.id}
                                type="button"
                            ...
                            >
                                ...
                                <span className="command-activity-panel__badge">
                ...
                <div className="command-activity-panel__activity">
                    ...
                    >
                        <span className="command-activity-panel__activity-title">
                            Recent commands
                            <span className="command-activity-panel__activity-count">
                                {activity.length}
                            </span>
                        </span>
                        {!activityExpanded && activitySummary ? (
                            <span className="command-activity-panel__activity-summary">
                                {activitySummary}
                            </span>
                        ) : null}
                        <ChevronDown
