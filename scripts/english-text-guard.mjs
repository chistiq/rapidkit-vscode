...
function violationKind(character) {
  ...
  if (codePoint === undefined || codePoint <= 0x7f || ALLOWED_LETTERLIKE_SYMBOLS.has(codePoint)) {
    return null;
  }
...
function readCandidate(repositoryRoot, file, mode) {
  try {
    if (mode !== 'staged') return fs.readFileSync(path.join(repositoryRoot, file));
...
export function scanRepository(options = {}) {
  ...
  for (const file of files) {
    ...
    violations.push(...findNonEnglishTextViolations(buffer.toString('utf8'), file));
  }
